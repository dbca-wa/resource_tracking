...
class DevicesView(GeojsonView):
    ...
    def get(self,request):
        ...
        geojson = serialize('geojson',q,geometry_field='point',srid=self.srid or "4326",properties=('id','deviceid','name','callsign','symbol','rego','make','model','category','heading','velocity','altitude','seen','age_minutes','age_colour','age_text','icon'))
        ...
        if deviceid:
            ...
            if start is None:
                ...
                try:
                    ...
                    #Failed to parse the start date, use the default value: one day before
                    start = timezone.now() - timedelta(days=1)
...
class HistoryView(LoggedPointView):
    ...
    def get_geojson(self,q):
        ...
        generate geojson data
        """
        return serialize('geojson',q,geometry_field='point',srid=self.srid or "4326" ,properties=('id','heading','velocity','altitude','seen','raw','device_id'))
    ...
    def get_geojson(self,q):
        ...
        for p in q:
            if start_point is not None:
                setattr(start_point,"route",LineString(start_point.point,p.point))
                setattr(start_point,"label","{0} to {1}".format(start_point.seen.strftime("%H:%M:%S") if start_point.seen else "",p.seen.strftime("%H:%M:%S") if p.seen else ""))
        ...
        return serialize('geojson',q,geometry_field='route',srid=self.srid or "4326" ,properties=('id','heading','velocity','altitude','seen','raw','device_id','label'))


geojson_patterns = patterns('',
