...
def table_exists(cur, schema, table_name, log=False):
    cur.execute(
        "SELECT count(*) FROM pg_catalog.pg_class a join pg_catalog.pg_namespace b on a.relnamespace = b.oid WHERE b.nspname='{}' and a.relname='{}' and a.relkind='r'".format(
            schema, table_name
        )
    )
    row = cur.fetchone()
    ...
    if log:
        print(
            "The table({}.{}) {} exist".format(
                schema, table_name, "does" if exists else "doesn't"
            )
        )
    return exists
    ...
    with conn.cursor() as cur:
        if not table_exists(cur, schema, table_name, log=log):
            ...
            try:
                ...
                if log:
                    print(
                        "Succeed to create the table({}.{}) with sql({})".format(
                            schema, table_name, create_sql
                        )
                    )
            except:
                print(
                    "Failed to create the table({}.{}) with sql({}).{}".format(
                        schema, table_name, create_sql, traceback.format_exc()
                    )
                )
